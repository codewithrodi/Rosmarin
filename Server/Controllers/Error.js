const { RuntimeError } = require('../Utilities/Runtime');
...
const ErrorHandlers = {
    ...
    OnDatabaseDuplicatedFields: (RaisedError) => {
        ...
        let Message = '';
        if(Name === 'Email')
            Message = 'There is already user with that email address, use another.';
        else if(Name === 'Username')
            Message = 'The username is already in use, try another.';
        else
            Message = `The field "${Name}" is already in use, try another value.`
        return [Message, 400];
    },
